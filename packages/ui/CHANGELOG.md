...
- 1d97922: Text-sized controls get a 24×24 hit area (WCAG 2.5.8 target size) from an invisible `::before` centred on the label, so the layout does not change: `Button variant="link"` and the Chart's "View as table" toggle.

## 1.0.1

### Patch Changes

- 420c66e: `DropdownMenuItem` / `ContextMenuItem` (and their sub-trigger, checkbox and radio
