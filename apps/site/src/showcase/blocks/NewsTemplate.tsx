          ...
          <nav aria-label={t('footer.legal')} className="flex gap-4">
            {LEGAL.map((l) => (
              <button
                ...
                type="button"
                onClick={() => onLegal(l)}
                className="hover:text-foreground focus-visible:ring-ring focus-visible:ring-offset-background relative rounded-sm outline-none before:absolute before:top-1/2 before:left-1/2 before:size-[max(100%,1.5rem)] before:-translate-x-1/2 before:-translate-y-1/2 before:content-[''] focus-visible:ring-2 focus-visible:ring-offset-2"
              >
                {legalTitle(t, l)}
              </button>
            ))}
